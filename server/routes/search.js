...
// GET /api/search?q=title&author=author
router.get('/', async (req, res) => {
  const { q, author } = req.query
  if (!q) return res.status(400).json({ error: 'Query parameter "q" is required' })

  try {
    const results = await searchProwlarr(q, author)
    res.json(results)
  } catch (err) {
...
// POST /api/request — grab a specific release (or create book + grab)
router.post('/request', async (req, res) => {
  ...
  if (autoGrab && topRelease) {
    try {
      ...
      console.error('[search/request] SABnzbd error:', err.message)
